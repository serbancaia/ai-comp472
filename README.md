...
## Our '.py' Python script files: 
#### In addition to the '.py' Python script files found in Part 1:
...
d) <br />      
  ...
  <br />  
  <b>NOTE: This file is not meant to be run. Its script is called through the cnnmodel_single_gen.py and cnnmodel_permutation_gen.py files</b>
<br />  
<br />
...
# Project Part 3
Updates on Dataset folders found in 'phase3' branch of our repository:
<br />
>GeneratedSplitBiasDataset(Part2dataset).zip: https://github.com/serbancaia/ai-comp472/blob/phase3/GeneratedSplitBiasDataset(Part2dataset).zip <br />
This dataset uses the 'test' folder of GeneratedSplitDataset from Part 2 (https://github.com/serbancaia/ai-comp472/blob/phase2/GeneratedSplitDataset.zip) to split the dataset into the 'Age' and 'Gender' bias attributes to perform the bias detection tasks<br />
<b>Note: This folder must be renamed to 'GeneratedSplitBiasDataset' for the script confusion_matrix_analysis_and_metrics_bias.py to work when evaluating this dataset </b>
<br /><br />
ProjectDatasets.zip: https://github.com/serbancaia/ai-comp472/blob/phase3/ProjectDatasets.zip <br />
This is the final complete dataset that includes dataset cleaning and bias mitigation. We used the datasetsplitter.py script to automatically divide our dataset's classes into 'train', 'validation', and 'test' folders. This dataset is different from the one found in the 'phase2' branch of our repository
><br /><br />
>GeneratedSplitDataset.zip: https://github.com/serbancaia/ai-comp472/blob/phase3/GeneratedSplitDataset.zip <br />
This is the final dataset that includes dataset cleaning and bias mitigation, and is split into train/validation/test folders. This was used for the training and testing of our final model. It was automatically generated by the datasetsplitter.py script from the ProjectDatasets.zip from Part 3. This dataset is different from the one found in the 'phase2' branch of our repository
><br /><br />
>GeneratedSplitBiasDataset.zip: https://github.com/serbancaia/ai-comp472/blob/phase3/GeneratedSplitBiasDataset.zip <br />
This is the final dataset that includes dataset cleaning and bias mitigation, and is split into folders for the bias attributes 'Age' and 'Gender'. 'Age' is further split into 'Young', 'Middle-Aged', and 'Senior', and 'Gender' is split into 'Male' and 'Female'. This was used for reanalyzing the bias of our dataset after the additions of new images and data cleaning
<br />

<b>You may find and download the final best model we saved of our CNN model in a Google Drive. This is the saved model we used to test, evaluate, and analyze: https://drive.google.com/drive/folders/1h2ADxC7kW9SJmMQtaOhpFLlsMn9rEo5r </b>
<br />
NOTE: Our MainCNNModel.py file may override our saved best model .pth file if a better model is found when executing the scripts. If this occurs, redownload the original .pth files from our Google Drive to use for evaluation in the confusion_matrix_analysis_and_metrics.py, confusion_matrix_analysis_and_metrics_bias.py,  files to generate the same numbers found in the Evaluation section of our report.
<br /><br />

## Our '.py' Python script files:  
#### In addition to the '.py' Python script files found in Part 1 and 2:
### ---STEPS FOR EXECUTING OUR CODE FOR (a) CNN MAIN MODEL AND K-FOLD MODEL AND (b) EVALUATION ---
Step 0) Open the Python environment with the above packages installed through the command prompt and navigate to the directory containing the python scripts.  
<br />
	a)  <br />
	-MainCNNModel.py: This script is our final updated Main CNN model's architecture and training file. The model with the lowest validation loss gets saved as the best model as 'main_best_model.pth' 
 
		Step 1) Execute >python MainCNNModel.py  
<br />		
	-KFoldCrossValidationMainCNNModel.py: This script implements k-fold cross-validation for training a CNN using PyTorch. It loads an image dataset, splits it into training, validation, and test sets, and trains the CNN model across multiple folds. The script 	saves the best-performing model for each fold based on validation accuracy. It also calls the confusion_matrix_analysis_and_metrics_KFold_Model.py script after each model training session for each fold to evaluate the performance metrics of each model before 	creating the next one. It also prints a full table of the performance metrics for every model that has been saved and tallies the average metrics of the 10 folds. After every fold training, the script saves a file called main_kfold_best_model_fold{i}.pth, where 	{i} is the specific fold at which the program has arrived (starting from integer 1 instead of 0), used to store a serialized version of the best K-Fold Cross-Validation CNN model that was created for a specific fold {i}. It also saves the computed performance 	metrics in a text file called "K-Fold Cross-Validation Performance Metrics.txt" dedicated for this specific purpose after every fold execution.
 
 		Step 1) Execute >python KFoldCrossValidationMainCNNModel.py  
 
b) <br />      
	-confusion_matrix_analysis_and_metrics_bias.py: This script is designed to handle a bias group analysis on a dataset split into subgroups. It evaluates a chosen model (from the MainCNNModel module) across different bias groups. It calculates various metrics 	such as accuracy, precision, recall, and F1-score for each bias group and overall. Provides detailed output per bias subgroup, showing specific confusion matrices and performance metrics in a structured tabular format.

		Step 1) Execute >python confusion_matrix_analysis_and_metrics.py  
		Step 2) Enter the file name of the saved main model (ex: "main_best_model.pth")
		Note: You may enter the file names with or without the file type '.pth'
<br />
	-confusion_matrix_analysis_and_metrics_KFold_Model.py: This script computes and analyzes performance metrics (precision, recall, F1 score, accuracy) for a CNN model trained using k-fold cross-validation. It evaluates the model’s predictions on a test dataset 	for a specific fold, generates a confusion matrix, and calculates metrics for each class and overall model performance. Results are printed to the console.
 	<br />  
	<b>NOTE: This file is not meant to be run. Its script is called through the KFoldCrossValidationMainCNNModel.py file</b>

## Dataset.pdf:  
<br />
	This file contains updates of our final dataset. <br /><br />
 	New: <br />
  	->Direct links to all of the datasets used throughout the entirety of the project, separated by project deliverables and includes short descriptions of each link	<br />
   	->Updated 25 sample images of each class found in our dataset
    	->Short description of changes made to the dataset in regards to bias detection and mitigation

## ProjectReport-Part3.pdf:  
<br />
	This file contains the complete report of our work from Part 1, Part 2, and Part 3 of our project.
<br /> <br />
 	From Part 1: The report includes a title page, information about the creation of our dataset, the explanation of our data cleaning techniques and challenges, the explanation of the labeling methods and 
	challenges, and the visualizations of our dataset. These visualizations include a bar graph to display the number of images our dataset 
	contains in each expression's training data class, one aggregrated pixel intensity distribution histogram for each expression's training 
	data class, and pixel intensity histograms for 15 randomly chosen images in each expression's training data class. Lastly, this report 
	contains a reference section for all the material used/referenced throughout this deliverable. 
	<br /><br />
 	From Part 2: The report includes the model overview and details of our CNN architecture, it's training process, and the evaluations of the Performance metrics between the main model and the 2 variants, the confusion matrix analyses of 
  	the 3 models, the impact of architectural variations, and our final conclusions and forward look of improvements for our model.
<br /> <br /> 
	<b>New additions from Part 3:</b> The final report includes updated information on each section of the report such as updates on the dataset in regards to addressing bias, data cleaning, labeling, updates of our CNN architecture, and evaluation of our new saved 	model through confusion matrices, K-fold cross-validation, and bias analysis on both our old main model and dataset from Part 2 and our new main model and dataset from Part 3

## Models Folder: 
<br />
	This folder is unchanged from Part 2 of the project. <br />
	
## model_count_config.txt: 
<br />
	This file is unchanged from Part 2 of the project. <br />

## Expectation of Originality files:  
<br />
	Expectations-of-Originality-June27-2024-AlinCaia.pdf: <br />
		This is the Expectation of Originality form signed by Alin Caia <br />
<br />
	Expectations-of-Originality-MelissaAnanian40112159.pdf: <br />
		This is the Expectation of Originality form signed by Melissa Ananian <br />
